...
mapLoop:
    lw t1, 0(s0)        # load the address of the array of current node into t1 line1
    lw t2, 4(s0)        # load the size of the node's array into t2

    slli t3, t0, 2      # line3
    add t1, t1, t3      # offset the array address by the count
    mv t3, t1           # t1 will be changed below.
    lw a0, 0(t1)        # load the value at that address into a0

    jalr ra, s1, 0      # call the function on that value. line2

    sw a0, 0(t3)        # store the returned value back into the array line4.
    addi t0, t0, 1      # increment the count
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    lw a0, 8(s0)        # load the address of the next node into a0 line 5.
    mv a1, s1           # put the address of the function back into a1 to prepare for the recursion another problem
...
printLoop:
    ...
    li a0, 11
    ecall
    lw a0, 8(t0) # a0 gets address of next node, cause t0 is the node address, 8(t0) is the next address.
