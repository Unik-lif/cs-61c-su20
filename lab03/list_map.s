...
.text
main:
    jal ra, create_default_list # the default list is a list of nodes taking stream 9876543210 as values.
    add s0, a0, x0  # a0 = s0 is head of node list

    #print the list
    add a0, s0, x0 # a0 is taking the start address of the list.
    jal ra, print_list
...
map:
    # Prologue: Make space on the stack and back-up registers # this will depend on how many saved registers we have used.
    ### YOUR CODE HERE ###
    addi sp, sp, -12
    sw ra, 0(sp) 
    sw s0, 4(sp)
    ...
    # THINK: why a0? cause we will call func!
    ### YOUR CODE HERE ###
    lw a0, 0(s0) 
...
loop:   #do...
    li  a0, 8
    jal ra, malloc      # get memory for the next node, malloc 8 bytes for the next node.
    sw  s1, 0(a0)   # node->value = i
    sw  s0, 4(a0)   # node->next = last
    add s0, a0, x0  # last = node, a0 stores the address, put the address to the s0, representing the new inserted node.
    addi    s1, s1, 1   # i++
    addi t0, x0, 10
